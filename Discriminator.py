# -*- coding: utf-8 -*-
# Reutilizado de PyWANN de Firmino (github) com pequenas alterações
import numpy as np
...
class Discriminator:
    ...
    def add_training(self, retina):
        ...
        for i in range(0, num_cicles, self.__num_bits_addr):
            bin_addr = self.__mapping_positions[i:i+self.__num_bits_addr]
            int_addr = np.sum([2**posi*retina[bin_addr[posi]]
                               for posi in range(self.__num_bits_addr)],
                              dtype=np.int64)
            mem_index = int(round(i/self.__num_bits_addr))
        ...
        if self.__retina_length % self.__num_bits_addr > 0:
            ...
            int_addr = np.sum([2**posi*retina[bin_addr[posi]]
                               for posi in range(num_rest_positions)],
                               dtype=np.int64)
    ...
    def get_DRASiW(self):
        ...
        for i in range(0, self.__retina_length, self.__num_bits_addr):
            ...
            for j in range(self.__num_bits_addr):
                posi = positions[j]
